        ...
        foreach ($transformedEntityMappings as $mapping) {
            ...
            foreach (clone $destinationEntity->properties as $index => $property) {
                if ($property instanceof SQLAttribute || $property instanceof SQLForeignKey) {
                    if (!$sourceEntity->properties->contains(fn(SQLProperty $e): bool => $e->propertyDescription->renamingIdentifier === $property->propertyDescription->renamingIdentifier) && ($statement = $adapter->newCreateColumnStatement($property, $destinationEntity->columnAfter($property instanceof SQLForeignKey && !$sourceEntity->properties->contains(fn(SQLProperty $e): bool => $e->propertyDescription->renamingIdentifier === $property->propertyDescription->renamingIdentifier) ? $destinationEntity->attributes->endIndex() : $destinationEntity->properties->indexBefore($index))))) {
                        $connection->execute($statement);
