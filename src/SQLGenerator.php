...
final class SQLGenerator
{
    ...
    private function predicateTraversesRelationships(Predicate $predicate): bool
    {
        if ($predicate instanceof ComparisonPredicate) {
            ...
            foreach ($analyser->keyPathExpressions as $keyPathExpression) {
                if ($keyPathExpression->operand instanceof SubqueryExpression) {
                    return false;
                }
    ...
    /**
     * @param ArrayClass<string> $columnNames
     */
    private function appendUniqueColumnsToSelectList(ArrayClass $columnNames): void
    {
        if ($columnNames->isEmpty) {
            return;
        }
    ...
    {
        $parentTableAlias = $this->tableReference;
        $aliasPathAccumulator = $this->tableReference;
        $currentSerialization = $this->request->serialization;
    ...
    private function keyPathExpressionsForFetchRequestDerivations(?Dictionary $serialization = null, ?string $parent = null, ?EntityDescription $entity = null): Set
    {
        ...
        foreach ($serialization as $key => $value) {
            ...
            // A traversal reads a column through the join that defines its alias, so it needs one emitted here. An aggregate becomes a correlated subquery carrying its own FROM, and joining its relationship as well as multiplies the rows the outer query carries while the subquery is re-evaluated over each one.
    ...
    private function keyPathExpressionsForFetchRequestPredicate(?Predicate $predicate = null): Set
    {
        ...
        if ($predicate instanceof ComparisonPredicate) {
            ...
            $analyser = new SQLPredicateAnalyser();
            $predicate->rightExpression->accept($analyser, PredicateVisitorFlags::all);
            if (!$analyser->keyPathExpressions->isEmpty) {
                $expressions->formUnion($analyser->keyPathExpressions);
            }
        } elseif ($predicate instanceof CompoundPredicate) {
    ...
    private function buildKeyPathExpression(Expression $expression, ?bool &$isDeterministic = true): string
    {
        ...
        foreach ($properties as $idx => $property) {
            if ($property instanceof SQLPrimaryKey || $property instanceof SQLEntityKey || $property instanceof SQLAttribute || $property instanceof SQLForeignKey) {
                if ($property instanceof SQLAttribute && ($expression = $property->derivationExpression) && $property->isRuntimeOnly) {
    ...
    private function buildAnySubquery(SQLToMany|SQLManyToMany $relationship, ComparisonPredicate $innerPredicate): string
    {
        ...
        return "EXISTS " . $this->buildCorrelatedSubqueryString($this->createSubQueryGenerator($relationship, null, $this->entity->primaryKey->columnName, $alias, $innerPredicate), $relationship, $this->tableReference, $alias, true);
    }
    ...
    {
        $alias = $this->aliasGenerator->generateTableAlias();
        return "$keyPath = " . $this->buildCorrelatedSubqueryString($this->createSubQueryGenerator($relationship, null, $this->entity->primaryKey->columnName, $alias, $innerPredicate), $relationship, $this->entity->tableName, $alias, true);
    }

    private function resolveRelationshipFromKeyPath(Expression $expression): SQLToMany|SQLManyToMany|null
    {
        $entity = $this->entity;
        $relationship = null;
    ...
    {
        $entity = $this->entity;
        $outerColumnReference = "$outerAlias.{$entity->primaryKey->columnName}";
        if ($relationship instanceof SQLToMany) {
            $innerColumnReference = "$innerAlias.{$relationship->inverseToOne->foreignKey->columnName}";
        } else {
