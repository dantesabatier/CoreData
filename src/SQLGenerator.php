...
class SQLGenerator extends ObjectClass
{
    ...
    private function startSQL(PersistentStoreRequest $request): void
    {
        if ($request instanceof FetchRequest) {
            ...
            if ((!$request->includesSubentities && $subentities->count > 1) || (!$entity->isPersistentHistoryEntity && (!$entity->isRootEntity && $subentities->count > 1) && (!$entity->superentity?->isRootEntity || $subentities->count > 1)) || ($entity->isRootEntity && !$entity->isAbstract && $subentities->count > 1)) {
