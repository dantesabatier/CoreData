...
class PersistentStoreCoordinator extends ObjectClass
{
    ...
    public static function registeredStoreTypes(): Dictionary
    {
        if (static::$registeredStoreTypes === null) {
            static::$registeredStoreTypes = new Dictionary([SQLStoreType => SQLCore::class, XMLStoreType => XMLObjectStore::class, BinaryStoreType => BinaryObjectStore::class, InMemoryStoreType => MemoryObjectStore::class]); // @phpstan-ignore-line
        }
    ...
    public function addPersistentStoreWithType(PersistentStoreType $storeType, ?string $configuration, URL $storeURL, ?Dictionary $options = null): PersistentStore
    {
         ...
         * @return bool
         * @throws Exception
         */ fn(string $class, string $type): bool => $type === $class::metadataForPersistentStore($storeURL)[StoreTypeKey]
     ...
     * @return ArrayClass<ManagedObject|ManagedObjectID|Dictionary<mixed>|Number>|ArrayClass<ArrayClass<ManagedObject|ManagedObjectID|Dictionary<mixed>|Number>> An array containing managed objects, managed object IDs, or dictionaries as appropriate for a fetch request; an empty array if request is a save request, or nil if an error occurred.
