                            ...
                            foreach ($keys as $key) {
                                ...
                                if ($property instanceof SQLColumn || $propertyDescription instanceof ExpressionDescription) {
                                    if ($current instanceof ArrayClass) {
                                        ...
                                        if (!$current->isEmpty) {
                                            $parent = $current->first(fn(Dictionary $dictionary): bool => $dictionary[$currentEntity->primaryKey->columnName] === $parentID && $dictionary[$currentEntity->entityKey->columnName] === $parentEntityName) ?? $current->first;
                                            $current = &$parent;
                                        }
                                    }
                                    if ($current instanceof Dictionary) {
