    ...
    protected(set) ArrayClass $createTableStatements {
        get {
            if (!isset($this->createTableStatements)) {
                ...
                if ($this->isUnique) {
                    $createTableStatements[] = new SQLStatement("ALTER TABLE `{$this->entity->tableName}` ADD CONSTRAINT `{$this->indexDescription->name}` UNIQUE INDEX IF NOT EXISTS ({$elements->join(", ")}) USING BTREE");
                } else {
                    $createTableStatements[] = new SQLStatement("ALTER TABLE `{$this->entity->tableName}` ADD INDEX IF NOT EXISTS `{$this->indexDescription->name}` ({$elements->join(", ")}) USING BTREE");
                }
